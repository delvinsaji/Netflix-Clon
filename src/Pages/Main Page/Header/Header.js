import React, { useState } from "react";
import Logo from "../../../Components/Logo/Logo";
import "./Header.css";

function Header() {
  const [dropdown, setDropdown] = useState("avatar_dropdown");
  const [username, setUsername] = useState(window.location.pathname.slice(10));
  return (
    <div className="header1">
      <div className="header2">
        <Logo st={1}></Logo>
        <div className="genre">
          <p className="genre_p">Home</p>
          <p className="genre_p">TV shows</p>
          <p className="genre_p">Movies</p>
          <p className="genre_p">My list</p>
        </div>
      </div>
      <div className="header3">
        <p style={{ color: "white", fontSize: " 15px" }}>Hi, {username}</p>
        <img
          onClick={() => {
            if (dropdown === "avatar_dropdown") {
              setDropdown("avatar_dropdown2");
            } else {
              setDropdown("avatar_dropdown");
            }
          }}
          className="avatar"
          src="data:image/png;base64,"
        ></img>
        <div className={dropdown}>
          <p>Profile</p>
          <p>Watchlist</p>
          <p>Logout</p>
        </div>
      </div>
    </div>
  );
}

export default Header;
